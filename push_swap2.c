...
/*void	pull_number_up(t_list **stack_a)
{
	int	len;
	int	min;
	int	pos;
...
void	first_partitions_to_b(t_list **stack_a, t_list **stack_b)
{
	int	i;
	int	pivot;

	pivot = get_pivot(*stack_a, ft_lstsize(*stack_a) - 1);
	printf("%i\n", pivot);
	i = 0;
...
void	sort_up_5(t_list **stack_a, t_list **stack_b)
{
	while (ft_lstsize(*stack_a) > 5)
		first_partitions_to_b(stack_a, stack_b);
	if (ft_lstsize(*stack_a) == 2 && !ft_lstsorted(*stack_a))
			move_sa(stack_a);
		else if (ft_lstsize(*stack_a) == 3 && !ft_lstsorted(*stack_a))
			sort_3(stack_a);
		else if ((ft_lstsize(*stack_a) == 4 || ft_lstsize(*stack_a) == 5)&& !ft_lstsorted(*stack_a))
			sort_4_5(stack_a, stack_b);
	while (stack_b)
	{
		pull_number_up(stack_b, ft_lstmaximum(*stack_b));
		move_pa(stack_a, stack_b);
	}
}
